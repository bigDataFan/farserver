...
import com.ever365.ecm.entity.Assoc;
...
public class PersonService {
	...
	public static final String R_ADMIN 	= "管理员";
	public static final String R_REG 	= "注册用户";
	public static final String R_FANS 	= "关注我的人";
	public static final String R_ALL 	= "所有人";
	public static final String R_FRIEND = "朋友";
	
	public static final List<String> RESERVED_USER = new ArrayList<String>();
	
	static {
		RESERVED_USER.add(R_ADMIN);
		RESERVED_USER.add(R_REG);
		RESERVED_USER.add(R_FANS);
		RESERVED_USER.add(R_ALL);
		RESERVED_USER.add(R_FRIEND);
	};
	...
	public boolean personExists(@RestParam(value="name")String userName) {
		if (RESERVED_USER.indexOf(userName)>-1) return true;
	...
	@RestService(method="POST", uri="/friend/add")
	public void addFriend(@RestParam(value="friends") List<String> friends)  { 
		
		Entity currentUser = getPeopleEntity(AuthenticationUtil.getCurrentUser());
		
		for (String friend : friends) {
			Entity friendEntity = getPeopleEntity(friend);
			if (friendEntity==null) continue;
			entityDAO.addEntityAssoc(currentUser.getId(), friendEntity.getId(), Model.ASSOC_FRIEND, friend);
			entityDAO.addEntityAssoc(friendEntity.getId(), currentUser.getId(), Model.ASSOC_FRIEND, AuthenticationUtil.getCurrentUser());
		}
		return;
	}
	
	@RestService(method="GET", uri="/friend/list")
	public List<String> getMyFriends()  { 
		Entity currentUser = getPeopleEntity(AuthenticationUtil.getCurrentUser());
		List<Assoc> assocs = entityDAO.getNodeAssocs(currentUser.getId());
		
		List<String> list = new ArrayList<String>();
		
		for (Assoc assoc : assocs) {
			list.add(assoc.getValue());
		}
		return list;
	}
	...
	public Entity createPerson(@RestParam(value="userId")String userName,
		...
		if (RESERVED_USER.indexOf(userName)>-1) {
		...
		Repository userRepo = repositoryDAO.addRepository(userRepoName, userName, "我的文件", conentStoreName);
	...
	public Entity getPeopleEntity(String userName) {
		...
		if (personEntity==null && ADMIN.equals(userName)) {
			...
			repositoryDAO.addRepository(Repository.REPOSITORY_DEFAULT, ADMIN, "公用文件夹", null);
		}
		return personEntity;
	}
	...
	public boolean checkPassword(@RestParam(value="name")String userName,
		...
		if (personEntity==null) {
			...
			if (!password.equals(personEntity.getProperty(Model.PROP_PASSWORD))) {
				throw new HttpStatusException(HttpStatus.UNAUTHORIZED);
			} else {
				return true;
			}
