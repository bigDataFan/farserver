...
public class RepositoryService {
	...
	@RestService(uri="/image/upload", method="POST", multipart=true)
	public Map<String, Object> uploadImage(
			@RestParam(value="file")InputStream is, @RestParam(value="size")Long size
			) {
		Repository imageRepo = repositoryDAO.getRepository(IMG_TEMP_REPO, true);
		
		Entity entity = createEmptyFile(imageRepo.getRootEntity().getId(), UUID.generate() + ".png", imageRepo);
		
		putStremToFile(is, size, imageRepo, entity);
	...
	@RestService(uri="/file/thumbnail", method="POST")
	public String attachFileThumbNail(
			@RestParam(value="id")String fileid,
			@RestParam(value="url")String url,
			@RestParam(value="width")Integer width,
			@RestParam(value="height")Integer height
	) {
		Entity entity = entityDAO.getEntityById(fileid);
		if (entity==null) {
		...
		String contentUrl = contentStore.putContent(new ByteArrayInputStream(url.getBytes()), url.length());
		String contentDataId = contentDAO.createContentData(repo.toString(), contentUrl, "image/png",
				new Long(url.length()), "UTF-8");
