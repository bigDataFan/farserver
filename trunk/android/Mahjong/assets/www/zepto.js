...
(function($) {
	...
	function add(element, events, fn, selector, getDelegate) {
		...
		eachEvent(
				...
				fn,
				function(event, fn) {
					var delegate = getDelegate && getDelegate(fn, event), callback = delegate
							|| fn;
					var proxyfn = function(event) {
						var result = callback.apply(element, [ event ]
								.concat(event.data));
						if (result === false)
			...
			add(element, event, callback, selector, function(fn) {
				return function(e) {
					...
					if (match) {
						...
						return fn.apply(match, [ evt ].concat([].slice.call(
								arguments, 1)));
					}
				}
			});
	...
	$.fn.on = function(event, selector, callback) {
		return selector === undefined || $.isFunction(selector) ? this.bind(
				event, selector) : this.delegate(selector, event, callback);
	};
	$.fn.off = function(event, selector, callback) {
		return selector === undefined || $.isFunction(selector) ? this.unbind(
				event, selector) : this.undelegate(selector, event, callback);
	};
	...
	$.fn.triggerHandler = function(event, data) {
		...
		this.each(function(i, element) {
			...
			e.data = data;
			e.target = element;
			$.each(findHandlers(element, event.type || event), function(i,
					handler) {
	...
	$.Event = function(type, props) {
		...
		event.initEvent(type, bubbles, true, null, null, null, null, null,
...
(function($) {
	function detect(ua) {
		var os = (this.os = {}), browser = (this.browser = {}), webkit = ua
				...
				&& ua.match(/TouchPad/), blackberry = ua
				.match(/(BlackBerry).*Version\/([\d.]+)/);
		...
		if (iphone)
			os.ios = true, os.version = iphone[2].replace(/_/g, '.'),
					os.iphone = true;
		if (ipad)
			os.ios = true, os.version = ipad[2].replace(/_/g, '.'),
					os.ipad = true;
		if (webos)
...
(function($) {
	...
	$.ajaxJSONP = function(options) {
		var callbackName = 'jsonp' + (++jsonpID), script = document
				.createElement('script'), abort = function() {
			$(script).remove();
	...
	$.ajax = function(options) {
		...
		var mime = settings.accepts[settings.dataType], baseHeaders = {}, protocol = /^([\w-]+:)\/\//
				.test(settings.url) ? RegExp.$1 : window.location.protocol, xhr = $.ajaxSettings
				.xhr(), abortTimeout;
	...
	$.fn.load = function(url, success) {
		...
		$.get(url, function(response) {
			self.html(selector ? $(document.createElement('div'))
					.html(response).find(selector).html() : response);
