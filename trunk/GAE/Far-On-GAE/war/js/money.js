	...
	*/
	...
	$.getJSON("/service/key/get", 
			...
			function(data) {
				...
				$('#networkInfo').html('您已经连接到服务器' + new Date(data.time).format("h:MM TT"));
				getRemoteKey(groupdb, 'money', "groupdb");
				getRemoteKey(incomedb, 'money', "incomedb");
				
				if (data.value!="[]") {
					categories = JSON.parse(data.value);
					if (window.localStorage!=null) {
						window.localStorage.setItem(currentUser+".category", data.value);
					}
				} 
...
function getRemoteKey(db, kind, key) {
	$.getJSON("/service/key/get",
			{"kind":kind, "key":key},
			function(data) {
				dbreg[key]().remove();
				...
				dbreg[key] = new TAFFY(list);
				
				/*
				
				for ( var j = 0; j < list.length; j++) {
		    		var record = data.value[j];
		    		db.insert(record);
				}
				*/
				...
				$('#' + key + 'size').html(dbreg[key]().count());
			});
}

function putToRemote(db, kind, key) {
	$.post("/service/key/update",
			{	
				"kind":kind, 
				"key": key,
...
var saveCurrentForm = function() {
	...
	form.each(
			function(data) {
				...
				if (extracted.___id) {
					...
					extracted.id = extracted.updated + "" + Math.floor(Math.random()*10000);
					dbreg[extracted.db].insert(extracted);
					$('#' + extracted.db + 'size').html(dbreg[extracted.db]().count());
				}
	...
	putToRemote(dbreg[form.attr("db")], "money", form.attr("db"));
	//putToRemote(incomedb, "money", "incomedb");
	...
	$('#detailList div.checked').each(
			function() {
				...
				if (data!=null) {
					...
					dbreg[data.db](data.___id).remove();
					$(this).remove();
				}
			}
	);
	if (odb!=null) {
		putToRemote(dbreg[odb], "money", odb);
	}
	//putToRemote(incomedb, "money", "incomedb");
...
function bindObject(div, o) {
	...
	if (o.id) {
		$(div).find('div.priority img').attr('src', "/image/online_dot.png");
	} else {
		$(div).find('div.priority img').attr('src', "/image/status_offline.png");
	}
}
