...
public class CopyMethod extends WebDAVMethod {
	...
	protected void executeImpl(HttpServletRequest req, HttpServletResponse resp)
		...
		String destPath = WebDAV.getDestinationPath(req);
		
		File file = fileService.getRootFile().getByPath(filePath);
		
		File targetParent = fileService.getRootFile().getByPath(StringUtils.getParentPath(destPath, WebDAV.PathSeperator));

		if (file!=null && targetParent!=null) {
			if (!file.isFolder()) {
				fileService.copyTo(targetParent.getDbObject(), file.getDbObject(), true);
			} else {
