...
public class RepositoryDAOImpl implements RepositoryDAO {
	...
	public Repository extractRepository(DBObject repodbo) {
		...
		if (repositoryCache.get(repodbo.get("_id").toString())==null) {
			...
			if (repodbo.get("desc")!=null) {
				repo.setDesc((String)repodbo.get("desc"));
			}
		}
	...
	@Override
	public Repository addRepository(String repository, String owner, String desc,
			String contentStore) {
		DBCollection repoColl = dataSource.getCollection(REPOSITORIES);
		...
		if (repoColl.findOne(dbo)==null) {
			...
			Map<QName, Serializable> auditableProperties = new HashMap<QName, Serializable>(1);
			auditableProperties.put(Model.PROP_OWNER, owner);
			
			Entity rootEntity  = entityDAO.addEntity(r, null, Model.FS_CONTAINS, null, Model.TYPE_FOLDER, "root", auditableProperties, specialPropties);
			Entity trashEntity = entityDAO.addEntity(r, null, Model.FS_CONTAINS, null, Model.TYPE_FOLDER, "trash", auditableProperties, specialPropties);
